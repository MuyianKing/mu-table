  ...
  setup(props, { emit }) {
    ...
    return () => (
      <table class="mu-table-header" cellspacing="0" cellpadding="0" border="0">
        <thead>
          <tr className="mu-table-tr">
            {
              (props.select) && (
                <th className="mu-table-td mu-table-th mu-table-checkbox" style="width:60px;min-width:60px;max-width:60px;text-align:center">
            ...
            {columns.value.filter(column => column.show).map((column, index) => (
              <th
                class={`mu-table-td mu-table-th ${column.className}`}
              ...
              >
                <div class="mu-table-td-div" style={{ justifyContent: align_map[column.style.textAlign] }}>
                  {column.renderHeader
                    ? column.renderHeader()
                    : (
