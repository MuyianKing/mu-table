            ...
            data.value.map((row, index) => {
              const arr = [
                <tr
                  ...
                  className={`mu-table-tr${props.activeIndex !== '' && +props.activeIndex === index ? ' active' : ''} ${index % 2 === 0 ? '' : 'enev'} ${props.rowClass && props.rowClass(row, index)}`}
                  onClick={() => {
                    rowClick(row, index)
                  }}
                ...
                >
                  {
                    (props.select) && (
                      <td className="mu-table-td mu-table-checkbox" style="width:60px;min-width:60px;max-width:60px;text-align:center">
                  ...
                  {columns.value.filter(column => column.show).map((column, col_index) => (
                    <td key={column.uuid} data-uuid={column.uuid} class={`mu-table-td ${column.className}`} style={column.tdStyle} onMouseenter={e => setTips(e, index, col_index)} onMouseleave={hideTip}>
                      {column.render({
                        row,
                        index,
                      })}
